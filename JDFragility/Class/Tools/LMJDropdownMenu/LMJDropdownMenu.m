...
//
    ...
    if(_strColor){
           [_mainBtn setTitleColor:_strColor forState:UIControlStateNormal];
    }else{
           [_mainBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    }


    [self.mainBtn setTitle:self.strs forState:UIControlStateNormal];
//    _mainBtn.buttonStyle=imageRight;
//[_mainBtn setImage:[UIImage imageNamed:@"Triangle Copy 6"] forState:UIControlStateNormal];
//    [_mainBtn setBackgroundImage:[UIImage imageNamed:@"Group 2-1"] forState:UIControlStateNormal];
...
//    _mainBtn.titleEdgeInsets    = UIEdgeInsetsMake(0, 15, 0, 0);
    ...
    if (self.bordrColor) {
        _mainBtn.layer.borderColor=self.bordrColor.CGColor;
    }
    ...
    // 旋转尖头
//    _arrowMark = [[UIImageView alloc] initWithFrame:CGRectMake(_mainBtn.frame.size.width - 25, 0, 19, 9)];
//    _arrowMark.center = CGPointMake(VIEW_CENTER_X(_arrowMark), VIEW_HEIGHT(_mainBtn)/2);
//    _arrowMark.image  = [UIImage imageNamed:@"kajixiala"];
//    [_mainBtn addSubview:_arrowMark];

}

-(void)setStrs:(NSString *)strs{
    _strs=strs;
        [self.mainBtn setTitle:strs forState:UIControlStateNormal];
}
...
- (void)showDropDown{   // 显示下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _mainBtn.imageView.transform = CGAffineTransformMakeRotation(M_PI);
        if(_rowHeight *_titleArr.count>500){
            _listView.frame  = CGRectMake(VIEW_X(_listView), VIEW_Y(_listView), VIEW_WIDTH(_listView),500);
        }else{
...
- (void)hideDropDown{  // 隐藏下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _mainBtn.imageView.transform = CGAffineTransformIdentity;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [_mainBtn setTitle:[NSString stringWithFormat:@" %@",cell.textLabel.text] forState:UIControlStateNormal];
    
    if ([self.delegate respondsToSelector:@selector(dropdownMenu:selectedCellNumber:str:)]) {
        [self.delegate dropdownMenu:self selectedCellNumber:indexPath.row str:cell.textLabel.text]; // 回调代理
    }
    if (self.block) {

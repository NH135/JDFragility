...
@property (nonatomic, copy) JXTAlerttextClickBlock textBlock;
//type
...
+ (void)showAlertViewWithTitle:(nullable NSString *)title
              ...
              otherButtonBlock:(nullable JXTAlerttextClickBlock)otherBlock{
    if (!(title.length > 0) && message.length > 0) {
        title = @"";
    }
    JXTAlertView *alertView = [[JXTAlertView alloc] initWithTitle:title message:nil cancelButtonTitle:@"取消" otherButtonTitle:@"确认", nil];

//    alertView.alertType = JXTAlertTypeNormal;
[alertView setAlertViewStyle:UIAlertViewStylePlainTextInput];
    UITextField *txtName = [alertView textFieldAtIndex:0];
    txtName.placeholder = @"请输入备注";
    txtName.text=message;


    alertView.buttonClickBlock = ^(NSInteger buttonIndex){
        if(otherBlock){
            otherBlock(alertView.textstr);
        }
